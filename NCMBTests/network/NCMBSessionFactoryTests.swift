...
final class NCMBSessionFactoryTests: NCMBTestCase {
    ...
    func test_getInstance() {
        NCMBSessionFactory.setInstance(session: DummySession())
        let dummy_session : NCMBSessionProtocol = NCMBSessionFactory.getInstance()
        XCTAssertNotNil(dummy_session as? DummySession)
    }
    ...
    private class DummySession : NCMBSessionProtocol {
