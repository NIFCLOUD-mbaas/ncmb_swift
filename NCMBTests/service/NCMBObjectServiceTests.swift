...
/// NCMBObjectService のテストクラスです。
final class NCMBObjectServiceTests: NCMBTestCase {
    ...
    func test_find_request() {
        ...
        let sut = NCMBObjectService()
        sut.find(query: query, callback: {result in
            XCTAssertEqual(executor.requests.count, 1)
            XCTAssertEqual(executor.requests[0].apiType, NCMBApiType.classes)
            XCTAssertEqual(executor.requests[0].method, NCMBHTTPMethod.get)
            XCTAssertEqual(executor.requests[0].subpathItems, ["TestClass"])
            ...
            XCTAssertEqual(try! executor.requests[0].getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/classes/TestClass?where=%7B%22takanokun%22:%2242%22%7D"))
    ...
    func test_fetch_request() {
        ...
        object.objectId = "abcdef012345"

        let expectation : XCTestExpectation? = self.expectation(description: "test_fetch_request")
        let sut = NCMBObjectService()
        sut.fetch(object: object, callback: {result in
            XCTAssertEqual(executor.requests.count, 1)
            XCTAssertEqual(executor.requests[0].apiType, NCMBApiType.classes)
            XCTAssertEqual(executor.requests[0].method, NCMBHTTPMethod.get)
            XCTAssertEqual(executor.requests[0].subpathItems, ["TestClass", "abcdef012345"])
            XCTAssertEqual(executor.requests[0].queryItems.count, 0)
            XCTAssertNil(executor.requests[0].body)
            XCTAssertEqual(try! executor.requests[0].getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/classes/TestClass/abcdef012345"))
            XCTAssertEqual(executor.requests[0].timeoutInterval, 10.0)
            expectation?.fulfill()
        })
        self.waitForExpectations(timeout:1.00, handler: nil)
    ...
    func test_save_request_post() {
        ...
        let sut = NCMBObjectService()
        sut.save(object: object, callback: {result in
            XCTAssertEqual(executor.requests.count, 1)
            XCTAssertEqual(executor.requests[0].apiType, NCMBApiType.classes)
            ...
            XCTAssertEqual(try! executor.requests[0].getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/classes/TestClass"))
            XCTAssertEqual(executor.requests[0].timeoutInterval, 10.0)
            expectation?.fulfill()
        })
        self.waitForExpectations(timeout:1.00, handler: nil)
    ...
    func test_delete_request() {
        ...
        let sut = NCMBObjectService()
        sut.delete(object: object, callback: {result in
            XCTAssertEqual(executor.requests.count, 1)
            XCTAssertEqual(executor.requests[0].apiType, NCMBApiType.classes)
    ...
    func test_createGetRequest_emptyObjectId() {
        ...
        object.objectId = nil
        let sut = NCMBObjectService()
        XCTAssertThrowsError(try sut.createGetRequest(object: object)) { error in
    ...
    func test_createGetRequest_success() {
        ...
        object.objectId = "abcdef012345"
        let sut = NCMBObjectService()
        let request : NCMBRequest = try! sut.createGetRequest(object: object)
        XCTAssertEqual(request.apiType, NCMBApiType.classes)
        XCTAssertEqual(request.method, NCMBHTTPMethod.get)
        XCTAssertEqual(request.subpathItems, ["TestClass", "abcdef012345"])
        XCTAssertEqual(request.headerItems.count, 0)
        XCTAssertEqual(request.queryItems.count, 0)
        XCTAssertNil(request.body)
        XCTAssertEqual(try! request.getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/classes/TestClass/abcdef012345"))
    ...
    func test_createGetRequest_query() {
        ...
        XCTAssertEqual(request.apiType, NCMBApiType.classes)
        XCTAssertEqual(request.method, NCMBHTTPMethod.get)
        XCTAssertEqual(request.subpathItems, ["TestClass"])
        XCTAssertEqual(request.headerItems.count, 0)
        XCTAssertEqual(request.queryItems.count, 1)
        XCTAssertEqual(request.queryItems["where"]!, "{\"takanokun\":\"42\"}")
        XCTAssertNil(request.body)
        XCTAssertEqual(try! request.getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/classes/TestClass?where=%7B%22takanokun%22:%2242%22%7D"))
        XCTAssertEqual(request.timeoutInterval, 10.0)
    }
    ...
    func test_createPostRequest_success() {
        ...
        let object = NCMBObject(className: "TestClass", fields: fields)
        object.objectId = nil // object have not objectId
        ...
        XCTAssertEqual(try! request.getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/classes/TestClass"))
        XCTAssertEqual(request.timeoutInterval, 10.0)
    }
    ...
    func test_createPutRequest_success() {
        let object = NCMBObject(className: "TestClass")
        object["takanokun"] = "takano_san"
        let sut = NCMBObjectService()
        let request : NCMBRequest = try! sut.createPutRequest(object: object, objectId: "abcdef012345")
        XCTAssertEqual(request.apiType, NCMBApiType.classes)
    ...
    func test_createDeleteRequest_emptyObjectId() {
        ...
        object.objectId = nil
        let sut = NCMBObjectService()
        XCTAssertThrowsError(try sut.createDeleteRequest(object: object)) { error in
            XCTAssertEqual(error as! NCMBInvalidRequestError, NCMBInvalidRequestError.emptyObjectId)
        }
    }
    ...
    func test_createDeleteRequest_success() {
        ...
        object.objectId = "abcdef012345"
        let sut = NCMBObjectService()
        let request : NCMBRequest = try! sut.createDeleteRequest(object: object)
        XCTAssertEqual(request.apiType, NCMBApiType.classes)
