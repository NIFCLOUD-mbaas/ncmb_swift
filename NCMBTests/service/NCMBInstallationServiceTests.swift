...
/// NCMBInstallationService のテストクラスです。
final class NCMBInstallationServiceTests: NCMBTestCase {
    ...
    func test_find_request() {
        ...
        sut.find(query: query, callback: {result in
            ...
            XCTAssertEqual(executor.requests[0].apiType, NCMBApiType.installations)
            ...
            XCTAssertEqual(try! executor.requests[0].getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/installations?where=%7B%22takanokun%22:%2242%22%7D"))
    ...
    func test_fetch_request() {
        ...
        installation.objectId = "abcdef012345"

        let expectation : XCTestExpectation? = self.expectation(description: "test_fetch_request")
        let sut = NCMBInstallationService()
        sut.fetch(object: installation, callback: {result in
            XCTAssertEqual(executor.requests.count, 1)
            XCTAssertEqual(executor.requests[0].apiType, NCMBApiType.installations)
            XCTAssertEqual(executor.requests[0].method, NCMBHTTPMethod.get)
            ...
            XCTAssertNil(executor.requests[0].body)
            XCTAssertEqual(try! executor.requests[0].getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/installations/abcdef012345"))
            XCTAssertEqual(executor.requests[0].timeoutInterval, 10.0)
            expectation?.fulfill()
        })
        self.waitForExpectations(timeout:1.00, handler: nil)
    ...
    func test_save_request_post() {
        ...
        sut.save(object: installation, callback: {result in
            ...
            XCTAssertEqual(try! executor.requests[0].getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/installations"))
            XCTAssertEqual(executor.requests[0].timeoutInterval, 10.0)
            expectation?.fulfill()
        })
        self.waitForExpectations(timeout:1.00, handler: nil)
    ...
    func test_delete_request() {
        ...
        let sut = NCMBInstallationService()
        sut.delete(object: installation, callback: {result in
            XCTAssertEqual(executor.requests.count, 1)
            XCTAssertEqual(executor.requests[0].apiType, NCMBApiType.installations)
    ...
    func test_createGetRequest_emptyObjectId() {
        ...
        installation.objectId = nil
        let sut = NCMBInstallationService()
        XCTAssertThrowsError(try sut.createGetRequest(object: installation)) { error in
    ...
    func test_createGetRequest_success() {
        ...
        installation.objectId = "abcdef012345"
        let sut = NCMBInstallationService()
        let request : NCMBRequest = try! sut.createGetRequest(object: installation)
        ...
        XCTAssertEqual(try! request.getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/installations/abcdef012345"))
    ...
    func test_createGetRequest_query() {
        ...
        XCTAssertNil(request.body)
        XCTAssertEqual(try! request.getURL(), URL(string: "https://mbaas.api.nifcloud.com/2013-09-01/installations?where=%7B%22takanokun%22:%2242%22%7D"))
        XCTAssertEqual(request.timeoutInterval, 10.0)
    }
    ...
    func test_createPutRequest_success() {
        ...
        let sut = NCMBInstallationService()
        let request : NCMBRequest = try! sut.createPutRequest(object: installation, objectId: "abcdef012345")
        XCTAssertEqual(request.apiType, NCMBApiType.installations)
    ...
    func test_createDeleteRequest_emptyObjectId() {
        ...
        installation.objectId = nil
        let sut = NCMBInstallationService()
        XCTAssertThrowsError(try sut.createDeleteRequest(object: installation)) { error in
            XCTAssertEqual(error as! NCMBInvalidRequestError, NCMBInvalidRequestError.emptyObjectId)
        }
    }
    ...
    func test_createDeleteRequest_success() {
        ...
        installation.objectId = "abcdef012345"
        let sut = NCMBInstallationService()
        let request : NCMBRequest = try! sut.createDeleteRequest(object: installation)
        XCTAssertEqual(request.apiType, NCMBApiType.installations)
